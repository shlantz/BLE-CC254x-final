...
#define BATTERY_CHARG   HAL_LED_6

#define WIRELESS_CHARG  HAL_LED_7
/*********************************************************************
 * TYPEDEFS
 */
...
static uint16 CellVoltage = 0;
static uint16 CellCurrent = 0;
static uint16 ToEmpty_min = 0;
static float percent = 0.00f;
//static uint8 i = 0x01;
static uint8 indic_LED = 0x00;
...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "Gizmo WirelessCharger";
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  CellCurrent = getCellCurrent_mA();
  ToEmpty_min = getTimeToEmpty_min();
  percent     = getStateOfCharge_f();
  
  HalLedSet( (BATTERY_CHARG), HAL_LED_MODE_ON );
  HalLedSet( (WIRELESS_CHARG), HAL_LED_MODE_OFF );
  ...
  // Setup the SimpleProfile Characteristic Values
  {
    ...
    uint8 charValue6[SIMPLEPROFILE_CHAR6_LEN] = { 1, 2, 3, 4, 5, 6, 7};
...
/*  
  #define HAL_LED_1 0x01 P10
  #define HAL_LED_2 0x02 P11
  #define HAL_LED_3 0x04 P12
  #define HAL_LED_4 0x08 P13
  #define HAL_LED_5 0x10 P14
*/
...
//    i<<=1; //Left Shift 1 bit
//    if(i>=0x20) //if great than 0x20 
//    {
//      i = 0x01;
//    }  
...
static void performPeriodicTask( void )
{
  ...
  if(CHARGE_STATE==0 && LEDS_STATE==0)
     sendValue_2 = 0x68;
  if(CHARGE_STATE==0 && LEDS_STATE==1)
     sendValue_2 = 0x7B;
  if(CHARGE_STATE==1 && LEDS_STATE==0)
     sendValue_2 = 0x7C;
  if(CHARGE_STATE==1 && LEDS_STATE==1)
     sendValue_2 = 0x7D;
  
  SimpleProfile_SetParameter( SIMPLEPROFILE_CHAR1, sizeof( uint8 ), &sendValue_2 );
  ...
  {
    count++;
    HalLedSet( (WIRELESS_CHARG), HAL_LED_MODE_ON );
    if(count>40)
      count = 0;    
  }
  */
...
static void simpleProfileChangeCB( uint8 paramID )
{
  ...
  switch( paramID )
  {
    case SIMPLEPROFILE_CHAR1:
      ...
      {
        CHARGE_STATE = 1;
        HalLedSet( (WIRELESS_CHARG), HAL_LED_MODE_ON );  //Wireless Charge function ON
      }
      if(newValue == 0x6F) 
      {
        CHARGE_STATE = 0;
        HalLedSet( (WIRELESS_CHARG), HAL_LED_MODE_OFF ); //Wireless Charge function OFF
      }
      
      if(newValue == 0x70) 
      {
        LEDS_STATE = 1;
        indic_LED = 0x01;  //Indication LEDS ON
      }
      if(newValue == 0x71) 
      {
        LEDS_STATE = 0;
        indic_LED = 0x00;  //Indication LEDS OFF
      }
      
      if(newValue == 0x39) // get battery status data
      {        
        //HalLedSet( (BATTERY_CHARG), HAL_LED_MODE_OFF );
        CellVoltage = getCellVoltage_mV();
        CellCurrent = getCellAverageCurrent_fmA();
        ToEmpty_min = getRemainingCapacity_mAh();
        percent     = getStateOfCharge_f();
        ...
        newValueBuf[0] = (uint8)(CellVoltage >> 8);     // MSB
        newValueBuf[1] = (uint8)(CellVoltage & 0x00ff); // LSB
        
        newValueBuf[2] = (uint8)(CellCurrent >> 8);
        newValueBuf[3] = (uint8)(CellCurrent & 0x00ff); 
        
        newValueBuf[4] = (uint8)(ToEmpty_min >> 8);
        newValueBuf[5] = (uint8)(ToEmpty_min & 0x00ff);
        
        newValueBuf[6] = (uint8)(percent);
        
        // Send battery status data to host
        SimpleProfile_SetParameter(SIMPLEPROFILE_CHAR6,sizeof(newValueBuf),&newValueBuf);
        /* 
           It is more appropriate to use GATT_Notification here.
           Because after executing this instruction, I did not receive any returned data.
        */
      }
